...
class VAD_mine(MVXTwoStageDetector):
    """VAD model.
    """
    def __init__(self,
                 use_grid_mask=False,
    ...
    def obtain_history_bev(self, img_dict, img_metas_dict):
        ...
        with torch.no_grad():
            for t in img_dict.keys():
                img = img_dict[t]
                img_metas = [img_metas_dict[k][t] for k in range(len(img_metas_dict))]
    ...
    def forward_train(self,
        ...
        # img_metas = input_['img_metas' ]
        # gt_bboxes_3d = input_['gt_bboxes_3d']
        # gt_labels_3d = input_['gt_labels_3d']
        # img = input_['img']
        # ego_his_trajs = input_['ego_his_trajs']
        # ego_fut_trajs = input_['ego_fut_trajs']
        # ego_fut_masks = input_['ego_fut_masks']
        # ego_fut_cmd = input_['ego_fut_cmd']
        # ego_lcf_feat = input_['ego_lcf_feat']
        # gt_attr_labels = input_['gt_attr_labels']
        # map_gt_labels_3d = input_['map_gt_labels_3d']
        # map_gt_bboxes_3d = input_['map_gt_bboxes_3d']
        # map_gt_bboxes_ignore = None
        # gt_bboxes_ignore = None

        batch_img_metas = []
        for i in range(len(img_metas)):
            a = (OrderedDict(sorted(img_metas[i].items())))
            batch_img_metas.append(OrderedDict([(int(k-self.queue_length+1), v) for k, v in a.items()]))
        
        img_dict = {}
        for ind, t in enumerate(batch_img_metas[0].keys()):
            img_dict[t] = img[:, ind, ...]
            
        img = img_dict[0]
        img_dict.pop(0)

        prev_img_metas = copy.deepcopy(batch_img_metas)
        for i in range(len(prev_img_metas)):
            prev_img_metas[i].pop(0)
        prev_bev = self.obtain_history_bev(img_dict, prev_img_metas)

        img_metas = [batch_img_metas[i][0] for i in range(len(batch_img_metas))]
    ...
    def forward_test(
        ...
        **kwargs
        ...
        print(self.prev_frame_info['prev_bev'])
        print("\n\n\n\n\n\n\n")
