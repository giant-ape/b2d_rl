...
from mmcv.models.backbones.resnet import Bottleneck, BasicBlock
import torch.utils.checkpoint as checkpoint
from mmcv.models.bricks import build_norm_layer, build_conv_layer
...
@TRANSFORMER.register_module()
class VADPerceptionTransformer_mine(BaseModule):
    """Implements the Detr3D transformer.
    Args:
        as_two_stage (bool): Generate query from encoder features.
            Default: False.
    ...
    def __init__(self,
                 ...
                 map_num_pts_per_vec=10,
                 **kwargs):
        super(VADPerceptionTransformer_mine, self).__init__(**kwargs)
        self.encoder = build_transformer_layer_sequence(encoder)
        if decoder is not None:
            self.decoder = build_transformer_layer_sequence(decoder)
        else:
