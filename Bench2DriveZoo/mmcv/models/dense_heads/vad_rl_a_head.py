...
from mmcv.utils import force_fp32, auto_fp16, load_checkpoint
...
class VADRLAHead(DETRHead):
    ...
    def __init__(self,
        ...
        if map_class_weight is not None and (self.__class__ is VADRLAHead):
        ...
        super(VADRLAHead, self).__init__(*args, transformer=transformer, **kwargs)
    ...
    def _init_layers(self):
        ...
        tl_feats_branch = []
        # tl_feats_branch.append(Linear(2048 * 10 * 13 + 6 * self.embed_dims * 12 * 20, self.embed_dims)) #10 * 13  5 * 7
        tl_feats_branch.append(Linear(23040, self.embed_dims)) #10 * 13  5 * 7
    ...
    def forward(self,
        ...
        # cf_img = cf_img[None,: :,:].repeat(batch,1, 1, 1)
        ...
        #     ego_cmdid_feat = self.ego_feat_projs[5](cmdid_onehot)
        ego_cmdid_feat = self.ego_feat_projs[5](command_id.to(dtype))

        ego_status = ego_lcf_feat.squeeze(1)[..., self.ego_lcf_feat_idx]
        ego_status_feat = self.ego_feat_projs[4](ego_status.to(dtype))
        ...
        outs = {
            ...
            'all_traj_preds': outputs_mot_trajs.repeat(outputs_coords.shape[0], 1, 1, 1, 1),
            'all_traj_cls_scores': outputs_mot_trajs_classes.repeat(outputs_coords.shape[0], 1, 1, 1),
            ...
            'tl_status_cls_scores': None,
            'tl_trigger_cls_scores': None,
            'stopsign_trigger_cls_scores': None,
            'ego_cls_col_preds': None,
            'ego_cls_bd_preds': None,
            'ego_cls_cl_preds': None,
    ...
    def loss_planning(self,
        ...
        loss_plan_cls_col = self.loss_plan_cls_col(
            ego_cls_col_preds.flatten(0, 2), plan_col_labels.flatten(),
        ...
        loss_plan_cls_bd = self.loss_plan_cls_bd(
            ego_cls_bd_preds.flatten(0, 2), plan_bd_labels.flatten(),
        ...
        loss_plan_cls_cl = self.loss_plan_cls_cl(
            ego_cls_cl_preds.squeeze(-1).flatten(0, 2), plan_cl_labels.flatten(),
        ...
        loss_plan_cls_expert = self.loss_plan_cls_expert(
            ego_cls_expert_preds.flatten(0, 2), plan_expert_labels.flatten(),
    ...
    def loss(self,
        ...
        loss_dict['loss_tl_status_cls'] = 0.
        loss_dict['loss_tl_trigger_cls'] = 0.
        loss_dict['loss_stopsign_trigger_cls'] = 0.
    ...
    def get_plan_expert_target(self,
        ...
        if ego_fut_masks[0] == 1.:
            ...
            traj_dis = torch.linalg.norm(ego_traj_preds[:,:,:] - ego_fut_gt.cumsum(dim=-2)[:,:,:], dim=-1).sum(dim=-1)
            plan_expert_labels[neg_idx] = 1
            plan_expert_labels_weight[neg_idx] = (torch.clip(traj_dis, min=0, max=100.) * 100.).to(ego_traj_preds.dtype)
