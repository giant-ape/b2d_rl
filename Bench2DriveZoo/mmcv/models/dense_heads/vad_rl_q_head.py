...
class VADRLQHead(DETRHead):
    ...
    def __init__(self,
        ...
        super(VADRLQHead, self).__init__(*args,transformer=transformer,**kwargs)
    ...
    def _init_layers(self):
        ...
        q_branch = []
        q_branch.append(Linear(self.embed_dims, self.embed_dims))
        q_branch.append(nn.LayerNorm(self.embed_dims))
        q_branch.append(nn.ReLU(inplace=True))
        q_branch.append(Linear(self.embed_dims, 1))
        self.q_branch = nn.Sequential(*q_branch)

        v_branch = []
        v_branch.append(Linear(self.embed_dims, self.embed_dims))
        v_branch.append(nn.LayerNorm(self.embed_dims))
        v_branch.append(nn.ReLU(inplace=True))
        v_branch.append(Linear(self.embed_dims, 1))
        self.v_branch = nn.Sequential(*v_branch)
    ...
    def forward(self,
        ...
        _tmp = pos2posemb2d(ego_plan_traj.reshape(1, 1 * self.fut_ts, -1), num_pos_feats=self.embed_dims // 2) \
                .reshape(1, self.fut_ts, -1)
        ego_query = _tmp.unsqueeze(0).repeat(bs, 1, 1, 1).reshape(bs, 1, -1)
        ego_query = self.ego_query_pre_branch(ego_query)

        if self.interaction_pe_type is not None:
            ego_agent_pos = torch.ones((bs, ego_query.shape[1], 2), device=ego_query.device)*0.5  # ego in the center
            ...
            ego_agent_pos = self.pe_embed_mlps(ego_agent_pos)
            ego_agent_pos = ego_agent_pos.permute(1, 0, 2)

        else:
            ego_agent_pos, = None

        ego_bev_query = self.ego_bev_decoder(
            query=ego_query.permute(1, 0, 2),
            key=bev_embed.permute(1, 0, 2),
            value=bev_embed.permute(1, 0, 2),
            query_pos=ego_agent_pos,
            key_pos=self.bev_pos_embedding.weight[:,None,:].repeat(1, bs, 1)[:bev_embed.shape[0]]
        )
        ...
        ego_bev_feat = self.ego_feat_projs[2](ego_bev_query)
        ego_feats = ego_status_feat + ego_bev_feat + ego_cf_feat + ego_wp_feat
        q_value = self.q_branch(ego_feats)
