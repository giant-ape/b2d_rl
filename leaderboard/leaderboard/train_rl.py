...
class LeaderboardEvaluator(object):
    ...
    def _setup_simulation(self, args):
        ...
        # cmd1 = f"{os.path.join(self.carla_path, 'CarlaUE4.sh')} -carla-rpc-port={args.port} -graphicsadapter={args.gpu_rank} -opengl -quality-level=Low"
        cmd1 = f"{os.path.join(self.carla_path, 'CarlaUE4.sh')} -carla-rpc-port={args.port} -graphicsadapter={args.gpu_rank} --low-memory -prefernvidia -benchmark -fps=15"
    ...
    def run(self, args):
        ...
        # while not crashed and self.manager.run_step < 1e6:
        #     config = route_indexer.get_random_config()
